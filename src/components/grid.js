...
const codeSearchCriteria = (props, criteria) => {
    return (
        codeTitleFilter(props.frontmatter.title, criteria) ||
        codeTagFilter(props.frontmatter.tags, criteria)
    )
}

const codeTagFilter = (tags, criteria) => {
    return tags.includes(criteria)
}

const codeTitleFilter = (title, criteria) => {
    ...
    return (
        <div className="grid justify-center items-center grid-cols-1 p-4 md:grid-cols-2 md:p-0 lg:grid-cols-3 xl:grid-cols-4">
            {code
                ...
                .filter(props => codeSearchCriteria(props, value))
                .map(props => {
                    return (
                        <Card
